...
Rules:
- Always JOIN to lookup tables to resolve human-readable names; never guess an ID.
...
def chat(request, app):
    ...
    sql = call_quickml_llm(app, SQL_SYSTEM_PROMPT, user_turn).strip()
    ...
    answer = call_quickml_llm(app, summary_prompt, summary_input)
...
def network(name, app):
    ...
    own_cases = zcql.execute_query(f"SELECT CaseMasterID FROM Accused WHERE AccusedName = '{name}'")
    case_ids = [str(r["CaseMasterID"]) for r in own_cases]
    if not case_ids:
        return jsonify({"nodes": [], "edges": []})

    case_id_list = ",".join(case_ids)
    co_accused = zcql.execute_query(f"""
        SELECT DISTINCT AccusedName, CaseMasterID FROM Accused
        WHERE CaseMasterID IN ({case_id_list})
    """)

    nodes, edges, seen = [], [], set()
    nodes.append({"id": name, "type": "primary"})
    for row in co_accused:
        other = row["AccusedName"]
        if other != name and other not in seen:
            seen.add(other)
            nodes.append({"id": other, "type": "associate"})
            edges.append({"source": name, "target": other, "via_case": row["CaseMasterID"]})
